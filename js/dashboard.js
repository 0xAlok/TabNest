...
document.addEventListener('DOMContentLoaded', () => {
  ...
  restoreSessionBtn.addEventListener('click', () => restoreSession(currentSessionId, false));
  restoreNewWindowBtn.addEventListener('click', () => restoreSession(currentSessionId, true));
...
function renderSessions() {
  if (currentSessions.length === 0) {
    ...
    `;
    document.getElementById('createFirstSessionBtn').addEventListener('click', showSaveForm);
    return;
  }
  
  // Apply filters and sorting
  let filteredSessions = filterSessionsBySearch(currentSessions, searchInput.value);
  ...
  if (viewMode === 'grid') {
    ...
    sessionsContainer.innerHTML = filteredSessions.map(session => createSessionListItem(session)).join('');
  }
  
  // Add event listeners to session cards/items
  document.querySelectorAll('.session-card, .session-list-item').forEach(element => {
    element.addEventListener('click', () => showSessionDetail(element.dataset.id));
  });
...
function showSessionDetail(sessionId) {
  ...
  modalSessionDate.textContent = `Date: ${new Date(session.createdAt).toLocaleString()}`;
  
  const tabCount = session.tabs.length;
  const groupCount = session.tabGroups.length;
  modalSessionStats.textContent = `Tabs: ${tabCount} ${groupCount > 0 ? `, Groups: ${groupCount}` : ''}`;
  ...
  session.tabGroups.forEach(group => {
    ...
    groupElement.innerHTML = `
      <div class="group-header" style="color: ${group.color}">
        ...
        <span class="group-count">${groupTabs.length} tab${groupTabs.length !== 1 ? 's' : ''}</span>
      </div>
    `;
...
function createTabItem(tab) {
  return `
    <div class="tab-item">
      <img class="tab-favicon" src="${tab.favicon || 'icons/icon16.png'}" onerror="this.src='icons/icon16.png'">
